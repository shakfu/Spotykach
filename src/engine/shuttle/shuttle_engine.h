...
public:
    ...
    // Four in-RAM tape tracks per deck. Shares tape's FX chain, hence the same FX labels.
    const char* param_label(ParamId id) const override {
        switch (id) {
            case ParamId::Speed:          return "varispeed";
            ...
            case ParamId::GritMix:        return "character";
