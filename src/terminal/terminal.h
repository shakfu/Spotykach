...
// NOTE the SPK_TERMINAL term: terminal.cpp includes this header OUTSIDE its own `#if SPK_TERMINAL`
// guard, so this file is parsed by every build. Without that term the guard fired on every USB_MIDI=1
// build - i.e. every BOOT_QSPI engine, incl. `make dist` - even though no terminal was being compiled.
// There is no conflict unless the channel is actually built.
