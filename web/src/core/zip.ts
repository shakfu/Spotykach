// zip.ts - a minimal ZIP writer, so the card builder works in browsers without File System Access.
//
...
// entire specification-relevant surface here is two fixed-layout headers and a CRC.
//
// Compression is INJECTED (the `Deflate` port) rather than imported: `CompressionStream` is a browser
// API, and everything else in this file is arithmetic. Passing it in keeps the writer pure and
// testable, and makes the fallback explicit - with no compressor every entry is stored, which is a
// bigger archive and a perfectly valid one.
//
...
import type { Deflate } from './ports.ts';
 ...
 * Build a ZIP archive.
 *
 * @param dirs empty directories to record, so a card's unused `radio/7` folders survive the round trip
 * @param deflate optional compressor; without one every entry is stored
 */
export async function makeZip(
  files: CardFile[], dirs: string[] = [], deflate?: Deflate,
): Promise<Uint8Array> {
  const entries: Array<{ name: string; bytes: Uint8Array; dir: boolean }> = [];
