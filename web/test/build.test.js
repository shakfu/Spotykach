...
import { suite, test, ok, eq, readJson, readWeb, sha256hex } from './harness.js';
import { makeLayout } from '../js/layout.js';
import { buildCard, missingFrom } from '../js/build.js';

suite('build');

const layout = makeLayout(readWeb('card_layout.json'));
const patches = readWeb('patches.json');
const manifest = readJson('build_manifest.json');
