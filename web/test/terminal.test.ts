...
import { suite, test, ok, eq, throws, rejects } from './harness.ts';
import {
  LineAssembler, isLog, parseReply, isDestructive, CommandError, Timeout,
  parseDescribe, vocabulary, parseUsbDiag,
} from '../src/core/protocol.ts';
import { Device } from '../src/core/device.ts';
import type { Transport } from '../src/core/ports.ts';
import { SerialTransport, requestPort, DAISY_VID } from '../src/platform/serial.ts';
...
test('classifies replies', () => {
  ...
  eq((throws(() => parseReply('err bad-deck'), CommandError) as CommandError).reason, 'bad-deck');
...
type Script = string[] | Array<string | string[]> | ((cmd: string) => string | string[] | undefined);

/**
 * A scripted device: answers each written command from a queue, or from a handler.
 *
 * It implements the full `Transport` port, including `onClose` and `info`, which is worth saying
 * because it did not always - and a fake that satisfies less than the real interface is a test that
 * checks a contract nothing ships.
 */
function fakeDevice(script: Script): {
  sent: string[];
  transport: Transport;
  ...
  return {
    ...
    transport: {
      async write(text: string) {
        ...
        const reply = typeof script === 'function'
          ? script(sent[sent.length - 1])
          : (script as Array<string | string[]>).shift();
        for (const line of ([] as string[]).concat(reply ?? [])) onLine(line);
      },
    ...
    fireClose: (reason: string) => onClose(reason),
  };
}
...
test('an err reply raises with the bare reason token', async () => {
  ...
  eq((await rejects(d.cmd('set param nope A 1'), CommandError) as CommandError).reason, 'unknown-param');
});
...
test('a late reply after a timeout does not offset every later command', async () => {
  ...
  // The sink is a constructor option now rather than a mutable field: the model supplies it once, so
  // a test that reached in and reassigned it was exercising a path production does not have.
  const d = new Device(f.transport, { timeout: 30, logSink: (l) => dropped.push(l) });
  await rejects(d.cmd('query empty A'), Timeout);
  f.emit('ok stale-answer'); // the first command finally replies
