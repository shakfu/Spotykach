...
import { suite, test, ok, eq, throws, readFixture, readJson, type Fixture } from './harness.ts';
import {
  writeWav, writeRaw, parseWav, readSamples, padToBytes,
  WavError, WavNeedMore, F32, INT16,
} from '../src/core/wav.ts';
...
test('a prefix too short for the header asks for more rather than reporting a broken file', () => {
  ...
  const e = throws(() => parseWav(full.subarray(0, 20), full.length), WavNeedMore) as WavNeedMore;
