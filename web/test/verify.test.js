...
import { suite, test, ok, eq, readJson, readWeb, entriesFromCase } from './harness.js';
import { makeLayout } from '../js/layout.js';
import { verifyCard, summarize } from '../js/verify.js';
import { writeWav, INT16, F32 } from '../js/wav.js';
...
test('reaches the same verdicts as sk_card.py on a deliberately-broken card', async () => {
  const c = readJson('verify_cases.json');
...
test('a card sk_card.py init just built has no errors', async () => {
  ...
  const c = readJson('clean_card.json');
  const found = await verifyCard(layout, { files: entriesFromCase(c), dirs: c.dirs });
