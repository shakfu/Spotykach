// verify.js - "why does my card not work?", in the browser.
//
...
//
// The input is deliberately abstract: an array of entries with a path, a size, and a lazy reader. That
// is satisfiable from a File System Access directory handle, a drag-and-dropped folder, or a plain
// <input webkitdirectory>, and by fixtures in the tests - none of which this file needs to know about.

/**
 * @typedef {object} CardEntry
 * @property {string} path  card-relative POSIX path, e.g. "tapes/tape_a_1.wav"
 * @property {number} size  bytes
 * @property {(max?: number) => Promise<Uint8Array>} read  the file, or its first `max` bytes
 */

/**
 * @typedef {object} Finding
 * @property {"error"|"warn"} level  "error" = will not work; "warn" = works, but probably not meant
 * @property {string} path
 * @property {string} problem
 * @property {string} fix
 */

import { parseWav, WavError, WavNeedMore } from './wav.js';
...
const finding = (level, path, problem, fix = '') => ({ level, path, problem, fix });
 ...
 * @returns {boolean} whether the file is visible enough to be worth format-checking
 */
...
async function checkAudioFormat(layout, entry, bank, out) {
  ...
  const problems = [];
  if (!fmt.encodings.includes(info.encoding)) problems.push(`encoding is ${info.describe().split(',')[0]}`);
 ...
 * @param {{files: CardEntry[], dirs?: Iterable<string>}} card
 * @returns {Promise<Finding[]>}
 */
export async function verifyCard(layout, card) {
  /** @type {Finding[]} */
  const out = [];
