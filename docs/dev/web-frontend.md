...
Status: **built, host-verified (2026-08-01).** Both phases landed in [`web/`](../../web); see
[What landed](#what-landed) for the outcome against this design, including the two places the build
deviates from it. In-browser DFU flashing remains out of scope (see [Non-goals](#non-goals)).

The design sections below ("Why" through "Testing") are kept **as originally written**, because the
three constraints they identify are still the reason the app is shaped the way it is; read them as the
brief, not as a description of the code. The closing "Open questions" and "Remaining verification"
sections are current. Read
[`../sd-card.md`](../sd-card.md) and [`terminal-control.md`](terminal-control.md) first; this is a
second front-end onto both, not new capability.
...
per-folder READMEs, the root README, the default config — because that text is a pure function of the
table, and it is the part a JS port would most plausibly reimplement and then let rot. The browser
therefore builds a card **byte-identical** to `sk_card.py init` (asserted per file by SHA-256) while
owning none of the wording. Two additions were pulled into `card_layout.py` to make the export
complete: the convert target-naming templates (`Bank.target`, replacing six per-engine branches in
`sk_card._target_names`) and the checker's shared rule sets (`SOURCE_EXTENSIONS`, `CONFIG_PROPERTIES`,
`SIDECAR_NAMES`, `SKIP_DIRS`).

**The parity guarantee is enforced, not asserted.** The table below listed five pieces to reimplement;
in the end only two are code rather than data, and both are pinned in both directions:
...
Synthesis was **not** ported (the design listed it as optional): the released `sk-card-<version>.zip`
is already a checksummed artifact, so the page links it rather than producing a second, unverifiable
copy of the same content.

**Phase 2 shipped in full but stays gated.** The console, the descriptor-generated control surface, the
CPU plot and the `query usb` table are all built and unit-tested against a scripted fake device — the
client is transport-agnostic for exactly that reason, so none of it needed hardware. The firmware
...
has been read through the File System Access API, and no device has been driven over WebSerial. Those
are the remaining checks.
...
- ~~**Where does the app live**~~ — this repo, under `web/`. `card_layout.json` is a build artifact of
  this repo, and the drift guard (`test_web_export.py`) only works if the two live together.
- ~~**Should the web app build demo cards at all**~~ — no. It builds the skeleton and links the
  released `sk-card-<version>.zip` for the audio, so the user gets the checksummed artifact rather than
  an unverifiable regeneration of it.
- **Does the resampling quality matter?** Still untested. `OfflineAudioContext` is fine for tape/radio
  material; for `pstretch` sources feeding a 50× stretch, artefacts have a long time to become audible.
  The Convert tab states the caveat rather than implying parity. Comparing a browser-converted clip
  against an ffmpeg-converted one at 50× is a bench-session item.
- **Is phase 2 worth it if the answer to constraint 2 is "do not ship terminal builds"?** Now a cheaper
  question than it was, since phase 2 is built and costs nothing to keep. But it stays close to useless
  to anyone who does not build their own firmware, so **constraint 2 is still the decision to make** —
  the code is no longer the thing standing in the way of it.
...
1. **Load the page in Chrome and in Safari.** The Safari run is the one that matters: it is the
   ...
   entirely untested against a real File API.
2. **Read a real card** through `showDirectoryPicker`, including a card with a `System Volume
   Information` folder on it, which the walk prunes.
3. **Convert a real mp3 and a real flac** and check the output on the device. The fixtures prove the
   *encoder* matches Python; they say nothing about `decodeAudioData`'s output being musically right.
4. **Drive a `TERMINAL=1` build over WebSerial.** The client is tested against a fake, so what is
   unverified is the transport: chunk boundaries from a real CDC endpoint, and whether the descriptor
   from a real engine generates a sensible surface.
