...
LABELS_JSON = Path(__file__).resolve().parent / "tosc_labels.json"
...
def load_labels(engine: str, path: Path = LABELS_JSON) -> dict[str, str]:
    """The curated layer-3 labels for one engine, or an empty map."""
    if not path.exists():
        return {}
    data = json.loads(path.read_text())
    return {k: v for k, v in data.get("engines", {}).get(engine, {}).items() if not k.startswith("$")}
...
def space_from_describe(text: str) -> AddressSpace:
    ...
    What a running device reports is authoritative, where the static parse is an inference and
    `tosc_labels.json` a copy that can rot - so this is the path to prefer when a device is to
    hand, and the only one that works for the Faust and gen~ engines, whose liveness masks are
    computed at run time from a binding table and cannot be read out of a header.

    The block is the line codec's, which is what `skdev` already parses and what a capture on the
    bench holds. Labels still come from the curated file: `describe` will carry them once the
    firmware implements the `param_label()` hook the design document proposes.
    """
    ...
    space = sk_osc.build_space(labels=load_labels(name))
...
def generate(
    ...
    *,
    ...
    if describe is not None:
        ...
        space = sk_osc.space_for(engine, labels=load_labels(name) if engine else {})
        space.engine = name
