...
def read_labels(engine: str, vocab: Vocabulary | None = None) -> dict[str, str]:
    """The engine's layer-3 labels, read out of its header - `{"param:speed": "station"}`.

    `IEngine::param_label()` is the authority, and it is written two ways:

    * a hand-written `switch (id) { case ParamId::Speed: return "station"; ... }` on most engines;
    * a walk over the Faust bind table on the generated ones, where the *slider* label is the
      layer-3 name (`filter` binds "cutoff" to `ParamId::Speed`). The table is a static array in
      the generated header, so it is read directly rather than through the walk.

    An engine with no labels - `granular`, deliberately, because the shared `ParamId` vocabulary is
    already its own words - gets an empty map and prints layer-2 names, which is what the firmware's
    own fallback does.
    """
    vocab = vocab or read_vocabulary()
    header = ENGINE_DIR / engine / f"{engine}_engine.h"
    ...
    labels: dict[str, str] = {}
    body = _mask_body(source, "const char* param_label(ParamId id) const override")
    if body:
        for member, text in re.findall(r'case\s+ParamId::(\w+)\s*:\s*return\s+"([^"]*)"', body):
            labels[f"param:{vocab.param_wire(member)}"] = text

    # The generated Faust engines. `setdefault` in source order gives stage A the role where a
    # chain binds one in both stages, which is the precedence `FaustChainEngine` implements.
    binds = re.findall(r'\{\s*[^,{}]*,\s*"([^"]*)"\s*,\s*static_cast<int>\(ParamId::(\w+)\)', source)
    for text, member in binds:
        if text:
            labels.setdefault(f"param:{vocab.param_wire(member)}", text)
    return labels
...
def space_for(
    ...
    *,
    ...
    Labels default to the engine's own `param_label()` table; pass `labels` to override, which is
    what a caller with a live `describe` in hand does.
    """
    vocab = read_vocabulary()
    if engine is not None and labels is None:
        labels = read_labels(engine, vocab)
...
#: The inventory this module exports for the off-target parity test to probe. Committed, and
#: regenerated by `--inventory`; `scripts/test_gen_tosc.py` fails if the committed copy has drifted.
INVENTORY = REPO / "host/osc_addresses.txt"

INVENTORY_HEADER = """\
# The layer-2 OSC address space, as scripts/sk_osc.py composes it from the firmware tables.
#
# GENERATED - do not edit. Regenerate with `scripts/sk_osc.py --inventory` (or `make tosc`), which
# scripts/test_gen_tosc.py checks is current.
#
# host/test_osc_addr.cpp reads this and puts every line through the REAL resolver
# (src/terminal/osc_addr.cpp): a composition rule this model gets wrong shows up there as
# `unknown-address` rather than as a control that silently does nothing on a bench. The same test
# asserts the reverse direction for the families `describe` enumerates - anything the device
# advertises and this file lacks is a rule the model is missing.
#
# One address per line: <address> <kind> <argtype>. Order is ParamId/ConfigId/table order.
"""


def write_inventory(path: Path = INVENTORY) -> str:
    """Render the unmasked address space as the fixture `host/test_osc_addr.cpp` probes."""
    space = build_space()
    lines = [f"{a.address} {a.kind} {a.argtype}" for a in space.all()]
    text = INVENTORY_HEADER + "\n".join(lines) + "\n"
...
if __name__ == "__main__":   # a quick look at what the firmware currently exposes
    ...
    if "--inventory" in sys.argv[1:]:
        write_inventory()
        print(f"{INVENTORY.relative_to(REPO)}: {len(build_space().all())} addresses")
        raise SystemExit(0)
