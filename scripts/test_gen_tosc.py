...
def test_labels_name_real_live_slots(vocab):
    data = json.loads(gen_tosc.LABELS_JSON.read_text())
    known = set(sk_osc.engines())
    for engine, labels in data["engines"].items():
        assert engine in known, f"{engine} is not an engine"
        mask = sk_osc.read_live_mask(engine, vocab)
        for key, label in labels.items():
            kind, _, slot = key.partition(":")
            assert kind in ("param", "cfg"), key
            pool = vocab.param_names if kind == "param" else vocab.config_names
            assert slot in pool, f"{engine}: {key} is not a layer-2 name"
            live = mask.params if kind == "param" else mask.configs
            if live is not None:
                assert slot in live, f"{engine}: {key} is labelled but not live"
            assert label == label.lower() and label.isascii(), f"{engine}: {label!r}"
            assert len(label) <= 16, f"{engine}: {label!r} is too long for a caption"


def test_labels_reach_the_caption():
    space = sk_osc.space_for("radio", labels=gen_tosc.load_labels("radio"))
...
def test_describe_and_the_static_parse_agree():
    ...
    from_source = sk_osc.space_for("radio", labels=gen_tosc.load_labels("radio"))
